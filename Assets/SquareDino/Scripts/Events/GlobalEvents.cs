...
public class GlobalEvents<T> : GlobalEvents
{
    ...
    private static GlobalEvents<T> EventSystemInstance => s_eventSystemInstance ?? (s_eventSystemInstance = new GlobalEvents<T>());
    ...
    public static event Action<T> Happened
    {
        add
        {
            if (EventSystemInstance.m_eventBackingDelegate == null)
                GlobalEventsMaitenance.RegisterNewEventSystem(EventSystemInstance);
...
public static class GlobalEventsMaitenance
{
